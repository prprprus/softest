...
async function bindNewTabListener(browser) {
  browser.on(event.newTab.type, async function (e) {
    ...
    const flag = await queue.eventClickTargetBlank.dequeueBlocking(page, 1000);
    console.log('===>', flag);
    ...
    if (flag != -1) {
      ...
      // parse operation 1 of `newTab` event
 ...
 * 
 * Normally, the callback of the `URLChange` event will be executed after
 * the `click` event, so need to use the queue for synchronization.
 * 
 * @param {puppeteer.Browser} browser - Browser instance launched via puppeteer.
 */
async function bindURLChangeListener(browser) {
  browser.on(event.URLChange.type, async function (e) {
    ...
    // If the return value is not equal to -1, It is operation 1,
    // otherwise it is operation 2 or operation 3.
    const info = await queue.eventClickTargetSelfCoordinates.dequeueBlocking(page, 3000);
    console.log('===> info recv ', info);
    // parse `clickTargetSelf` event
    if (info != -1) {
      // Parse `clickTargetSelf` event (operation 1 of `URLChange` event).
      // The operation 1 will destroy the original document, so cannot parse the
      // XPath in the callback of the `click` event and need to parse the XPath here.
      const res = await common.genTemporaryBrowser(queue.eventClickTargetSelfOldPage.dequeue());
      const stmt = await parser.parseClickTargetSelf(res[1], info);
      await sender.sendData(stmt);
      await res[1].waitFor(500);
      await common.closeTemporaryBrowser(res[0]);
    } else {
      ...
      // parse operation 2 or operation 3 of `URLChange` event
