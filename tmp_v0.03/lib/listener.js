...
async function bindURLChangeListener(browser) {
  browser.on(event.URLChange.type, async function (e) {
    ...
    if (info != -1) {
      // Since the operation 1 will destroy the original document, so cannot parse the
      // XPath in the callback of the `click` event and need to parse the XPath here.
      console.log('=========================>fuxk2:', queue.oldPage.getFirstElement());

      // =======================================================================>
      // 封装到 common，封装成一个临时 pptr，复用它，不用每次都创建一个 pptr
      ...
      // await bindClickListener(tmpPage);
      await common.setViewport(tmpPage, 2540, 1318);
      await tmpPage.goto(queue.oldPage.dequeue(), {
        waitUntil: 'networkidle0',
      });
      // const xpath = await common.getXPathByElement(tmpPage, info);
      // console.log('=========================>fuxk3:', xpath);
      let stmt = await parser.parseClickTargetSelf(tmpPage, info);
